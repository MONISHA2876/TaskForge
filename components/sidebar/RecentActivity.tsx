import { ACTIVITY_ITEMS } from "@/lib/placeholder-data";
import type { ActivityItem } from "@/types";

const dotStyles: Record<ActivityItem["type"], { bg: string; border: string; color: string }> = {
          ...
          return (
            <div key={item.id} style={{ display: "flex", gap: "12px", padding: "10px 0", position: "relative" }}>
              {idx < ACTIVITY_ITEMS.length - 1 && (
                <div style={{ position: "absolute", left: "15px", top: "36px", bottom: "-10px", width: "1px", background: "#F3F4F6" }} />
              )}
              <div style={{
                width: "30px", height: "30px", borderRadius: "50%", flexShrink: 0,
                display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px",
                background: dot.bg, border: `1.5px solid ${dot.border}`, color: dot.color,
              }}>
                {dotIcons[item.type]}
              </div>
              <div style={{ flex: 1, minWidth: 0, paddingTop: "4px" }}>
                <p style={{ fontSize: "13px", fontWeight: 500, color: "#111827", lineHeight: 1.45 }}>{item.action}</p>
                <p style={{ fontSize: "11px", color: "#9CA3AF", marginTop: "3px" }}>{item.time}</p>
              </div>
            </div>
          );
        })}
